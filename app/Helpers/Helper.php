...
if (!function_exists('showRawStatus')) {
    function showPageStatus($page,$type){
        ...
        switch($status){
            ...
            break;
            case 'done': 
                $badge = ($type === 'sfx' && auth()->id() === 1) ? '<label class="btn btn-success btn-xs  text-monospace '.$type.'-detail" data-url="'.route('file-manager.bufferImage',['volume_id'=>$page->volume_id,'type'=>config('lfm.vol')[$type],'fileName'=>$page->filename]).'"><i class="fas fa-images"></i> Done: '.$user.' </label><label class="btn btn-xs btn-outline-primary border-0 sync-preview" data-sync="'.route('pages.syncPreview',['page_id'=>$page->id]).'"><i class="fas fa-sync"></i></label>' : '<label class="btn btn-success btn-xs  text-monospace '.$type.'-detail" data-url="'.route('file-manager.bufferImage',['volume_id'=>$page->volume_id,'type'=>config('lfm.vol')[$type],'fileName'=>$page->filename]).'"><i class="fas fa-images"></i> Done: '.$user.' </label>' ;
            break;
            default:
