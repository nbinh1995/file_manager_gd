...
if (!function_exists('showRawStatus')) {
    function showPageStatus($page,$type){
        ...
        switch($status){
            case 'doing':
                $badge = $type === 'check' ? '<label class="btn btn-warning btn-xs text-monospace">Reject: '.$user.'</label> <label class="btn btn-xs btn-outline-danger border-0" data-toggle="popover" title="Reject Note" data-content="'.($page->note ?? '...').'"><i class="fas fa-question-circle"></i></label>' :'<label class="btn btn-warning btn-xs text-monospace">Doing: '.$user.'</label>';
            break;
            case 'done': 
