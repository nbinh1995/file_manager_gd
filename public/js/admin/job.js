...
$(document).ready(function () {
    ...
    function getTotalPrice(search = '') {
        $.ajax({
            url: 'ajax/ajaxGetTotalThisMonthView',
            method: 'post',
            data: {
                'Search': search,
            },
    ...
    function datatables() {
        ...
        //get total price on search
        jobTable.on('draw.dt', function () {
            getTotalPrice(jobTable.search());
            destroyEventKeyUp();
            status_input = true;
        });
        ...
        $('.dataTables_filter input[type=search]').on('keyup', function (event) {
            if (event.keyCode === 13) {
                jobTable.search(this.value).draw();
            }
        });

        jobTable.on('responsive-resize', function (e, datatable, columns) {
            let parent = $('.child');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data['message'] === 'success') {
                    jobTable.ajax.reload(completeDelete, false);
                } else {
                    closeModal();
    ...
    $('html body').on('keyup', 'input[name=PriceYen]', function (event) {
        exRate(result => {
            if(result.rate!= -1){
                let usd = $('#jobTable').find('input[name=Price]');
                let price = Math.round(result.rate * $(this).val());
                $(usd).val(price);
                $(usd).closest('label').text(formatNumber(price));
            }
        });
    });

    $('#jobTable').on('click', 'td.dtr-control', function () {
    ...
    function updateCellDataTable(row, name_col, value_col, id = null) {
        let rowData = jobTable.row($(row)).data();
    ...
    function detectOneEditCellAndSave() {
        if ($('#jobTable tbody').find('input:not([name=Paid]):visible , select:visible , textarea:visible')[0]) {
            saveCell($('#jobTable tbody').find('input:not([name=Paid]):visible , select:visible , textarea:visible'));
        }
    }
    ...
    function destroyEventKeyUp() {
        if ($('#jobTable tbody').find('input:not([name=Paid]):visible , select:visible , textarea:visible')) {
            $('#jobTable tbody').find('input:not([name=Paid]):visible , select:visible , textarea:visible').each(
