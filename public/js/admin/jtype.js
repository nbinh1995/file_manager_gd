...
const URL_SHOW = "jtypes/list";
const ID_TABLE = "#common-table";
const ID_MODAL = "#common-modal";
let jtypeTable;
var jtype = jtype || {};
jtype.setModal = function (titleModal, btnName, sizeModal, idForm, contentModal) {
...
jtype.createItem = function (url_create) {
    return $.ajax({
        url: url_create,
        method: "GET",
        dataType: "json",
...
jtype.storeItem = function (element, message = "Created Success!") {
...
jtype.editItem = function (url_edit) {
    return $.ajax({
        url: url_edit,
        method: "GET",
        dataType: "json",
...
jtype.updateItem = function (element, message = "Updated Success!") {
    let url = $(element).attr("action");
    let data = new FormData(element);
    return $.ajax({
        url: url,
...
jtype.destroyItem = function (url_delete, message = "Removed Success!") {
...
$(document).ready(function () {
    ...
    $(document).on("click", ".create", function (e) {
        ...
        jtype.createItem($(e.target).data("url"));
        $(ID_MODAL).modal("show");
    });

    $(document).on("click", ".edit", function (e) {
        let row = $(`#${TABLE}-${$(e.target).data('id')}`);
        jtype.editItem($(e.target).data("url")).done(function (data) {
    ...
    $(document).on("click", ".remove", function (e) {
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    jtype.destroyItem($(e.target).data("url"));
                }
            },
        });
    ...
    $(document).on("submit", `#${ID_FORM_CREATE}`, function (e) {
        e.preventDefault();
        jtype.storeItem(e.target).done(function (data) {
            $(ID_MODAL).modal("hide");
    ...
    $(document).on("submit", `.${CLASS_FORM_EDIT}`, function (e) {
        ...
        jtype.updateItem(e.target).done(function (data) {
            row.find(`.Name-${TABLE}`).empty();
